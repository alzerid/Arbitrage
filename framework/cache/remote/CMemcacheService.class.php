...
class CMemcacheService extends \Framework\Base\CService
{
	static protected $_SERVICE_TYPE = "memcache";  //Service type
	...
	{
		//Require Driver
		$this->requireServiceFile('CMemcacheDriver');
		$this->_drivers = array();
	}
	...
	public function getDriver($opt_prop=array())
	{
		...
		if(!isset($this->_drivers[$key]))
			$this->_drivers[$key] = new CMemcacheDriver($config->$key);
